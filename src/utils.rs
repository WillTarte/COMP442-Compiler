...
/// # Outputs
/// * A `TokenFragment`
pub(crate) fn parse_number(input_fragment: &str) -> TokenFragment { //FIXME doesnt parse floats properly
...
/// # Outputs
/// * A `TokenFragment`
pub(crate) fn parse_op_or_punct(input_fragment: &str) -> TokenFragment { //FIXME parentheses and such
...
pub(crate) fn parse_string(input_fragment: &str) -> TokenFragment {
    if input_fragment.as_bytes()[0] as char != '"'
    {
        ...
        {
            None => { panic!("Tried to parse string from the beginning of the input") }
